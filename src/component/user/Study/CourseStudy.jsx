...
const CourseStudy = () => {
        ...
        const handleSeek = () => {
                if (playerRef.current) {
                        playerRef.current.seekTo(0.75); // Điều hướng đến tiến độ cụ thể
                }
        };
        ...
        useEffect(() => {
                // Start measuring load time when videoURL changes
                setLoadStartTime(new Date().getTime());

                // Clean up function to reset loadStartTime
                return () => {
        ...
        const handleOnReady = () => {
                ...
                if (loadStartTime) {
                        ...
                        const videoLoadTime = loadEndTime - loadStartTime;
                        if (videoLoadTime > 7000) { // Kiểm tra nếu videoLoadTime lớn hơn 10 giây (10000 milliseconds)
                                // Thực hiện các hành động nếu videoLoadTime lớn hơn 10 giây
                                console.log('Video load time greater than 10 seconds:', videoLoadTime);
                                // Ví dụ: có thể thực hiện một số hành động khác tại đây
                                openModalRedirect();
                        }
                        setLoadTime(videoLoadTime);
                        setLoadStartTime(null); // Reset load start time
                }
        };

        const handleOnStart = () => {
                console.log('handleOnReady: ', progressToSeek);
                if (playerRef.current) {
                        playerRef.current.seekTo(progressToSeek);
                }
        };
        ...
        const loadLessionandCommentData = async () => {
                try {
                        ...
                        setVideoIndexArray(VideoData[resultIndex].indexArray);

                        handleVideoURL(VideoData[resultIndex].video_filepath, VideoData[resultIndex].progress);
                        setProgressToSeek(VideoData[resultIndex].progress);
                        ...
                        VideoData.forEach((video) => {
                                ...
                                if (VideoData[resultIndex].videoId === video.videoId) {
                                        ...
                                        setSectionIndexProgress(groupedVideos[sectionIndex]);
                                }
        ...
        //
        const loadDataUpdate = async (sectionIndexFunction) => {
                try {
                        console.log('load data update');
                        console.log('section Index Function loadDataUpdate: ', sectionIndexFunction);
                        const encodedUserId = localStorage.getItem("userId");
                        const userID = parseInt(atob(encodedUserId), 10);
                        ...
                        // Xử lý dữ liệu video
                        const videoList = videoResponse.data;
                        const VideoData = videoList.filter((item) => !item.isDeleted).sort((a, b) => new Date(a.createAt) - new Date(b.createAt)).map((item, index) => ({
                                ...item,
                                indexArray: index, // Thêm thuộc tính indexArray tương ứng với index của mỗi phần tử
                        }));
                        ...
                        // Sắp xếp mảng dựa trên sectionIndex
                        sectionArray.sort((a, b) => a.sectionIndex - b.sectionIndex);
                        sectionArray[sectionIndexFunction].isShowVideo = true;
                        setSectionData(sectionArray);
        ...
        // update sum viewed
        const updateSumViewed = (sectionIndexProgress) => {
                const updatedSectionInfo = [...sectionInfo];
                if (updatedSectionInfo[sectionIndexProgress].sumViewed !== updatedSectionInfo[sectionIndexProgress].sumVideo) {
                        updatedSectionInfo[sectionIndexProgress].sumViewed += 1;
                }
        ...
        const [progressToSeek, setProgressToSeek] = useState(0);
        ...
        const updateProgressVideo = async () => {
                const updatedProgressVideo = [...progressVideo];

                updatedProgressVideo[videoIndexArray] = true;
        ...
        const handleVideoURL = async (videoURL, progress) => {
                let url;
                ...
                // handleToProgressVideo(progress);
        }

        // tiến độ
        const [played, setPlayed] = useState(0);
        const [videoIndexArray, setVideoIndexArray] = useState('');
        const [sectionIndexProgress, setSectionIndexProgress] = useState('');
        const handleProgress = (state) => {

                setPlayed(state.played);

                if (state.played > 0.8 && !progressVideo[videoIndexArray]) {
                        console.log('check finish!');
                        checkIsFinishLession();
                        const updatedProgressVideo = [...progressVideo];
                        ...
                        setProgressVideo(updatedProgressVideo);
                        // updateProgressVideo();
                        // updateSumViewed(sectionIndexProgress);
                }
        };

        const checkIsFinishLession = async () => {
                console.log("trackingProgress");
                ...
                try {
                        ...
                        if (userIdLocal) {
                                ...
                                console.log(responseTrackProgress.data);
                                console.log(responseTrackProgress.status);
                                if (responseTrackProgress.status === 201) {
                                        console.log("tracking lession thành công!");
                                        loadDataUpdate(sectionIndexProgress);
                                }
        ...
        const updateProgress = async (videoID, sectionIndex) => {
                try {
                        const userIdLocal = localStorage.getItem("userId");
        ...
        //hiển thị panel thông báo chuyển hướng sang chế độ hỗ trợ mạng yếu        
        const [modalRedirect, setIsModalRedirect] = useState(false);

        function openModalRedirect() {
                setIsModalRedirect(true);
        }
        ...
        return (
                <div>
                        ...
                        <div style={{ display: 'flex', height: '100vh', flexDirection: 'row' }}>
                                <div style={{ backgroundColor: 'white', height: '100%', width: '75%', }}>
                                        ...
                                        <div style={{ height: '65%', backgroundColor: '' }}>
                                                {/* {isLoadingVideo && <div>Loading...</div>} */}
                                                {isVideoError ? (
                                                        <div>
                                                                <p>Kết nối không ổn định, Vui lòng chuyển sang chế độ dành cho kết nối yếu để học tiếp!</p>
                                                        </div>
                                                ) : (
                                                        <ReactPlayer
                                                                ref={playerRef}
                                                                ...
                                                                config={{
                                                                        file: {
                                                                                attributes: {
                                                                                        controlsList: 'nodownload', // Ngăn chặn nút tải xuống
                                                                                },
                                                                        },
                                                                }}
                                                        />
                                ...
                                <div style={{ height: '100%', width: '25%' }}>
                                        ...
                                        <p>Played: {played.toFixed(2)}</p>
                                        <button onClick={handleSeek}>Seek to {Math.round(0.75 * 100)}%</button>
                                        <p>Thời gian load</p>
                                        {loadTime && (
                                                <p>Video load time: {loadTime} milliseconds</p>
                                        )} */}
                                        <hr />
                                        {sectionData && (
                                                                                                ...
                                                                                                <div
                                                                                                        onClick={() => {
                                                                                                                if (video.indexArray !== lessionIndex) {
                                                                                                                        const previousVideoID = videoID;  // Lưu lại videoID hiện tại
                                                                                                                        const previousVideoIndexArray = videoIndexArray;
                                                                                                                        console.log('videoID: ', previousVideoID);
                                                                                                                        console.log('videoIndexArray: ', previousVideoIndexArray);
                                                                                                                        console.log('status: ', !progressVideo[videoIndexArray]);
                                                                                                                        // Gọi hàm cập nhật progress cho videoID hiện tại nếu nó tồn tại
                                                                                                                        if (previousVideoID && previousVideoIndexArray !== undefined && !progressVideo[videoIndexArray]) {
                                                                                                                                console.log('updateProgress');
                                                                                                                                setPlaying(false);
                                                                                                                                setTimeout(() => {
                                                                                                                                        updateProgress(previousVideoID, sectionIndex);
                                                                                                                                }, 1500);
                                                                                                                        }
                                                                                                                        setLessionIndex(video.indexArray);  // để tô màu video đang xem
                                                                                                                        setTitleVideo(lessionData[video.indexArray].title);
                                                                                                                        setProgressToSeek(video.progress);
                                                                                                                        handleVideoURL(video.video_filepath, video.progress);
                                                                                                                        ...
                                                                                                                        setVideoIndexArray(video.indexArray);
                                                                                                                        setSectionIndexProgress(sectionIndex);
                                                                                                                        console.log('sectionIndex onClick: ', sectionIndex);
                        ...
                        {/* modal notification redirect */}
                        <Modal
                                isOpen={modalRedirect}
                                onAfterOpen={afterOpenModal}
                                onRequestClose={closeModalRedirect}
                                style={customStyles}
                                contentLabel="Example Modal"
                        >
                                <h2 style={{ fontWeight: 'bold', color: 'black', flex: '1' }} ref={(_subtitle) => (subtitle = _subtitle)}></h2>
                                ...
                                <div style={{ width: '350px', paddingTop: 10 }}>
                                        <p>Kết nối mạng bị yếu, hãy chuyển hướng sang chế độ học tập dành cho kết nối mạng yếu!</p>
                                </div>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', }}>
                                        <button
                                                onClick={() => handleNavigate(`/user/course/study-network-support/${courseId}`)}
                                                style={{ width: '70%', padding: '10px', marginTop: '5px', borderRadius: '5px', backgroundColor: '#007bff', color: 'white', border: 'none', cursor: 'pointer', fontSize: '16px', fontWeight: 'bold', boxShadow: '0px 2px 4px rgba(0, 0, 0, 0.1)', transition: 'background-color 0.3s' }}>
